...
layui.use(['table', 'form', 'layer', 'laydate', 'element'], function () {
    ...
    var els = {
        ...
        kpiCards: document.getElementById('dash-kpi-cards'),
        refreshBtn: document.getElementById('dash-refresh-btn')
    };
    ...
    form.on('select(dashCommunity)', function (data) {
    ...
    function renderKpiCards(metrics) {
        ...
        for (var i = 0; i < metrics.length; i++) {
            var m = metrics[i];
            var iconClass = icons[m.key] || 'layui-icon-chart';
            ...
            html += '<div class="layui-col-md3 layui-col-sm6">'
                + '<div class="layui-card" style="cursor:pointer;" data-metric-key="' + (m.key || '') + '">'
                + '<div class="layui-card-header">'
                + '<i class="layui-icon ' + iconClass + '" style="margin-right:5px;"></i>'
                + '<span class="metric-name" data-key="' + (m.key || '') + '">' + escapeHtml(m.name || m.key || '') + '</span>'
                + '</div>'
                + '<div class="layui-card-body" style="padding:15px;">'
                + '<div style="font-size:28px; font-weight:bold;">' + escapeHtml(formatValue(m.value, m.format)) + '</div>'
                + '<div style="font-size:13px; margin-top:5px;' + changeClass + '">'
                + changeArrow + ' ' + changeVal + ' vs previous period'
                + '</div>'
                + '</div>'
                + '</div>'
                + '</div>';
        ...
        // Bind metric name clicks for definition drawer
        var nameEls = els.kpiCards.querySelectorAll('.metric-name');
    ...
    function openMetricDefinition(metricKey) {
        ...
        layer.open({
            ...
            content: '<div style="padding:20px;">'
                + '<div style="text-align:center;"><i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i> Loading...</div>'
                + '</div>',
                ...
                SiteOps.request('GET', '/api/v1/analytics/metrics/' + encodeURIComponent(metricKey))
                    .then(function (res) {
                        ...
                        body.html(
                            ...
                            + '<h3>' + escapeHtml(def.name || metricKey) + '</h3>'
                            + '<p style="margin:10px 0; color:#666;">' + escapeHtml(def.description || 'No description available.') + '</p>'
                            ...
                            + '<tr><td><strong>Source</strong></td><td>' + escapeHtml(def.source || 'N/A') + '</td></tr>'
